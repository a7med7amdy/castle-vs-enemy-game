...
private:
	...
	void drawPavedTower(REGION region, int paved) const;

public:
	GUI();
	~GUI();

	void PrintCastleInfo(Queue<string>&q);
	// Input Functions  ---------------------------
	...
	string GetString() const; // reads a string (keyboard input) from the user
							  ...
							  // Output Functions  ---------------------------
	void PrintMessage(int font, string msg,int y=0,int x=10) const; // y: change the height  , font: change font 
	...
	void drawPaved(Castle* castle) const;
	MenuItem getAction(bool clearQueue = true);
};
